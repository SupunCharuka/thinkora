...
export default function Latest() {
    ...
    // Small presentational card so each item can manage its own image error state
    function LatestCard({ blog }) {
        ...
        if (imgSrc && !imgSrc.startsWith('http') && !imgSrc.startsWith('//')) {
            ...
            if (base) imgSrc = `${base.replace(/\/$/, '')}${imgSrc}`;
            // otherwise leave as relative path and let Next rewrites handle it
        }

        return (
            <article key={blog._id || blog.slug} className="relative rounded-lg overflow-hidden shadow-md transform transition-all duration-300 ease-out group hover:shadow-xl hover:scale-105">
                    ...
                    <div className="w-full h-[420px] relative rounded-lg overflow-hidden bg-gray-100">
                        {imgSrc && !imgFailed ? (
                            ...
                            <div className="absolute inset-0 bg-gray-200 flex items-center justify-center">
                                <svg className="w-12 h-12 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            ...
            <div className="mt-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {loading ? (
                        ...
                        (items || []).slice(0, visibleCount).map((blog) => (
                            <LatestCard key={blog._id || blog.slug} blog={blog} />
                        ))
                    )}
                </div>
