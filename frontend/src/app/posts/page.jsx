...
function PostCard({post, variant = 'large'}){
	...
	if(variant === 'compact'){
		return (
			<article className="flex gap-3 items-start bg-white rounded-lg p-3 shadow-sm">
	...
	return (
		<article onClick={goToPost} onKeyDown={(e)=>{ if(e.key==='Enter') goToPost(); }} role="button" tabIndex={0} className="relative rounded-lg overflow-hidden shadow-lg cursor-pointer">
			<div className="block relative h-[420px] md:h-[420px]">
				<Image src={post.image} alt={post.title} fill className="object-cover" />
			</div>
			...
			<div className="absolute left-5 bottom-5 right-5 text-white">
				<h3 className="text-lg md:text-2xl font-bold leading-tight mb-2">{post.title}</h3>
